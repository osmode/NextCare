...
#define CONSUMER_KEY_WITHINGS         @"dd2e42f334187e59080776fcccbff2f229619f135ef99ee64b2f031135"
#define CONSUMER_SECRET_WITHINGS      @"bd38cff362e6c80330753ecc86d88569b926055c63bbb41b0b44d5288f"
...
#define API_URL_SERVER                @"http://162.243.36.210/"
...
@interface ServerCHQueryStringPair : NSObject
...
@implementation ServerCHQueryStringPair
...
extern NSArray * ServerCHQueryStringPairsFromDictionary(NSDictionary *dictionary);
extern NSArray * ServerCHQueryStringPairsFromKeyAndValue(NSString *key, id value);

NSString * ServerCHQueryStringFromParametersWithEncoding(NSDictionary *parameters, NSStringEncoding stringEncoding)
{
    NSMutableArray *mutablePairs = [NSMutableArray array];
    
    for (ServerCHQueryStringPair *pair in ServerCHQueryStringPairsFromDictionary(parameters))
    {
...
NSArray * ServerCHQueryStringPairsFromDictionary(NSDictionary *dictionary)
{
    return ServerCHQueryStringPairsFromKeyAndValue(nil, dictionary);
}

NSArray * ServerCHQueryStringPairsFromKeyAndValue(NSString *key, id value)
{
    ...
    if([value isKindOfClass:[NSDictionary class]]) {
        ...
        [[[value allKeys] sortedArrayUsingDescriptors:[NSArray arrayWithObject:sortDescriptor]] enumerateObjectsUsingBlock:^(id nestedKey, NSUInteger idx, BOOL *stop) {
            ...
            if (nestedValue)
            {
                [mutableQueryStringComponents addObjectsFromArray:ServerCHQueryStringPairsFromKeyAndValue((key ? [NSString stringWithFormat:@"%@[%@]", key, nestedKey] : nestedKey), nestedValue)];
            }
        }];
    } else if([value isKindOfClass:[NSArray class]]) {
        [value enumerateObjectsUsingBlock:^(id nestedValue, NSUInteger idx, BOOL *stop) {
            [mutableQueryStringComponents addObjectsFromArray:ServerCHQueryStringPairsFromKeyAndValue([NSString stringWithFormat:@"%@[]", key], nestedValue)];
        }];
    } else {
        [mutableQueryStringComponents addObject:[[ServerCHQueryStringPair alloc] initWithField:key value:value]];
    }
...
@interface ServerOAuthController ()
...
@implementation ServerOAuthController
...
- (void)obtainRequestTokenWithCompletion:(void (^)(NSError *error, NSDictionary *responseParams))completion
{
    ...
    NSString *parametersString = ServerCHQueryStringFromParametersWithEncoding(mutableParameters, NSUTF8StringEncoding);
    ...
    parametersString = ServerCHQueryStringFromParametersWithEncoding(mutableParameters, NSUTF8StringEncoding);
                ...
                completion:(void (^)(NSError *error, NSDictionary *responseParams))completion
{
    ...
    NSString *parametersString = ServerCHQueryStringFromParametersWithEncoding(allParameters, NSUTF8StringEncoding);
    ...
    parametersString = ServerCHQueryStringFromParametersWithEncoding(allParameters, NSUTF8StringEncoding);
                             ...
                             oauthSecret:(NSString *)oauth_token_secret
{
    ...
    NSString *request_url = API_URL_SERVER;
    ...
    */
    ...
    NSString *parametersString2 = ServerCHQueryStringFromParametersWithEncoding(allParameters, NSUTF8StringEncoding);
    NSString *request_url2 = API_URL_SERVER;
    ...
    if (queryParameters) {
        queryString = ServerCHQueryStringFromParametersWithEncoding(queryParameters, NSUTF8StringEncoding);
    }
    if (queryString) {
